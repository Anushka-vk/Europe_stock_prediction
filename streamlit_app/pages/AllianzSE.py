...
st.set_page_config(page_title="AllianzSE Stock Prediction")

st.markdown("<h1 style='text-align: center;'><em>AllianzSE STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>AllianzSE</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "AllianzSE"].copy()
...
fig.update_layout(title="AllianzSE Stock Price Prediction",
