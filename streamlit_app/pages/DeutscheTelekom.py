...
st.set_page_config(page_title="DeutscheTelekom Stock Prediction")
st.markdown("<h1 style='text-align: center;'><em> DeutscheTelekom STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>DeutscheTelekom</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "DeutscheTelekom"].copy()
...
fig.update_layout(title="DeutscheTelekom Stock Price Prediction",
