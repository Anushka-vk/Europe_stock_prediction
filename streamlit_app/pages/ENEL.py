...
st.set_page_config(page_title="ENEL Stock Prediction")

st.markdown("<h1 style='text-align: center;'><em>ENEL STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>ENEL</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "ENEL"].copy()
...
fig.update_layout(title="ENEL Stock Price Prediction",
