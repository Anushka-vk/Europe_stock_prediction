...
st.set_page_config(page_title="IntesaSanpaolo Stock Prediction")

st.markdown("<h1 style='text-align: center;'><em>IntesaSanpaolo STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>IntesaSanpaolo</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "IntesaSanpaolo"].copy()
...
fig.update_layout(title="IntesaSanpaolo Stock Price Prediction",
