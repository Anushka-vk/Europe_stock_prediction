...
st.set_page_config(page_title="Novartis Stock Prediction")

st.markdown("<h1 style='text-align: center;'><em>Novartis STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>Novartis</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "Novartis"].copy()
...
fig.update_layout(title="Novartis Stock Price Prediction",
