...
st.set_page_config(page_title="RocheHoldng Stock Prediction")

st.markdown("<h1 style='text-align: center;'><em>RocheHoldng STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>RocheHoldng</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "RocheHoldng"].copy()
...
fig.update_layout(title="RocheHoldng Stock Price Prediction",
