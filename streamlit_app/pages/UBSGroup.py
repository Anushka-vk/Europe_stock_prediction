...
st.set_page_config(page_title="UBSGroup Stock Prediction")

st.markdown("<h1 style='text-align: center;'><em>UBSGroup STOCK EXCHANGE</em></h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center;'><u>UBSGroup</u></h2>", unsafe_allow_html=True)

df = pd.read_csv("../STREAMLIT_APP/EuropeFGIS2.csv",parse_dates=['Date'], index_col='Date')  # Load stock data
x = df[df["Symbol"] == "UBSGroup"].copy()
...
fig.update_layout(title="UBSGroup Stock Price Prediction",
